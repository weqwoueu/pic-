Module ModuleMCCSigma
    ...
    contains
      ...
      subroutine  SigmaNormalization(SN,SO,GO,Inited)
                  ...
                  !GO%Name="Ar"
                  !Filename="./gas/lxcat/"//Trim(GO%Name)//"/"//Trim(SO%Name)//Trim(GO%Name)//".txt"
                  Filename="./MCC_jw/gas/lxcat/"//Trim(GO%Name)//"/"//Trim(SO%Name)//Trim(GO%Name)//".txt"  !$ mb.ZWZ 2021/12/19
                   ...
                   Do j=1,Nr
                                   ...
                                   do i=1,NSigmaRaw
                                    ...
                                    do i=1,Ns
                                        ...
                                        Else
                                            Call Locate(TempRaw%EnergySigma(1,:),NSigmaRaw,NEnergy,k) !$ find the left point of the interval that NEnergy locate in through dichotomy
                                            If (k<1) Then
                                            ...
                                            SN%Sigma(j,i)=S2*TempRaw%EnergySigma(2,k)+S1*TempRaw%EnergySigma(2,k+1) !$ interpolate the sigma(impact area) through linear weighting
                                        end if
                                    end do
