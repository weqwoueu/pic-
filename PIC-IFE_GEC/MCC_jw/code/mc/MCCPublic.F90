Module ModuleMCCPublic
    ...
    contains
      Subroutine MCC(Ns,Ng,PB,SO,GO,MCCB) 
        ...
        !Call PBIGlobal%Init
        !Call PBIGlobal%Init(PB,Ns)
        Call PBIGlobal%Init(PB(0),MCCB(0,1)%CollisionRatio)
        ...
        Do i=1,Ng
            do j=0,Ns!Ns
                If(MCCB(j,i)%Model/=0_4) then
                    ...
                    !Call SelectParticle(PB(j),1.0_8) !Select Particle to collision
                    do k=1,NCollision
                        Call MCCParticleBundle(k)%POI%VelRes(PB(j)%VFactor)  !$ rescale the velocity of particle (to real data )
                        Call MCCParticleBundle(k)%Updater(SO(j),GO(i))       !$ initialize the POT and update the MCC collision information (relative velocity, energy and so on)
                        ...
                        Select case (MCCB(j,i)%Model)
                            ...
                            Case(3_4)
                                ...
                                End If    
                        End select
                        Call MCCParticleBundle(k)%POI%VelRes(1.d0/PB(j)%VFactor)  !$ rescale the velocity of particle to original data
                    ...
                    Do k=PB(j)%NPar,1,-1
						If (ParticleAnnihilationIndex(k)) Then
                                Call PB(j)%DelOne(k)
                                PB(j)%nLoss(5) = PB(j)%nLoss(5) + 1 !> ab.ZWZ
						End If 
                    End Do
                        !End If
           ...
           Call MCCAddNewParticle(Ns,PB,PBIGlobal)
			   !PB%ReNumAMC = PB%NPar*PB%Weight*PB%dx
        return
     End  Subroutine MCC
            ...
            subroutine SelectParticle(PB,CollisionRatio)
                    ...
                    Associate(NPar=>PB%NPar)
                            ...
                            End If
                            !Do i = 1,NCollision
                            !    MCCParticleIndex(i) = i        ! ab.ZWZ when only passing collision eletron
                            !End Do
            ...
            subroutine SelectIndex(Nindex,Nselect,POIndex)
                    ...
                    Integer(4),intent(inout),Allocatable :: POIndex(:)
                    Integer :: Temp

                    Integer(4) :: i,j,k
                    !POIndex=(/(i,i=1,Nindex)/)      !$ initialize Particle Index array as /1, ..., Npar/
                    Do i = 1,Nindex
                        POIndex(i) = i
                    End Do
                    
                    Do i=1,Nselect                  !$ select Nselect particles' index to lie in front of the POIndex array
                          Call Random_Number(R)
                          k=(i+1)+(Nindex-(i+1))*R
                          Temp=POIndex(i)
                          POIndex(i)=POIndex(k)
             ...
             Subroutine MCCAddNewParticle(Ns,PB,PBI)
                    ...
                    !Associate(NPar=>PBI%NPar,POI=>PBI%POI,Index=>PBI%POI(i)%Index)
                            Do i=1,PBI%NPar
                                 ...
                                 PB(index)%nGen(3) = PB(index)%nGen(3) + 1 !> ab.ZWZ
                             End Do
                   !ENd Associate
                    return
