Module ModuleGrid
    ...
    subroutine InitializationGrid1D(GD,CF,Name,dx)
        Implicit none
     ...
     subroutine DumpGrid1D(GD,Mode)
        ...
        Character(len=99) :: Filename,Fname
        ...
        If (Mode==0) Then
            ...
            GD%Period = GD%Period + 1
            NameIndex = GD%Period
        End If 
                WRITE(fname,"(I9.6)") NameIndex
                !Write(filename,*) "./OUTPUT/EPF/Grid1D",NameIndex,".dat"
                Write(filename,*) "./OUTPUT/EPF/"//Trim(GD%Name)//Trim(fname)//".dat"
                Write(*,*) "Saving ",Filename," Please wait..."
                open (10,file=filename)
                Write(10,"(A50)") 'Variables = "Energy(eV)" "PartNum" "PartFrac"'
                Ns=GD%Ns
                do i=1,GD%Nx
		...
		Subroutine UpdateGrid2D(GD,Nx,A1D,Shift)
         ...
         Integer(4) :: NZip,N0,N1,Index
			Real(8) :: TempValue(1:GD%Nx)
                 N0=Mod(GD%Timer,GD%Period)
                 If (GD%Period>GD%Ny) Then
                         NZip=int(GD%Period/GD%Ny)
                         N1=NZip*GD%Ny
                         If (Shift>=1.and.Shift<=GD%Ns) Then
                                 If (N0<N1) Then
                                    Index=N0/NZip
                                    Call AnyAvg(A1D, TempValue(1:GD%Nx))
												GD%Value(1:GD%Nx,Index,Shift) = GD%Value(1:GD%Nx,Index,Shift)+TempValue(1:GD%Nx)
                                Else
                                    Index=GD%Ny
                                    Call AnyAvg(A1D, TempValue(1:GD%Nx))
												GD%Value(1:GD%Nx,Index,Shift) = GD%Value(1:GD%Nx,Index,Shift)+TempValue(1:GD%Nx)
