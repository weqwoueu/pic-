SUBROUTINE FUN_Density_rhs_2D(rho_, phi_, region, f_, x, y, n)
...
IF (n /=4) THEN
PRINT*, ' n/=4, Check FUN_Density_rhs_2D.f90, STOP'
STOP
ENDIF
...
IF (region == -1) THEN
	DO i=1,SIZE(phi_)
		f_(i) = rho_(i)
        !f_(i) = -4
!        f_(i) = rho_(i) +(x(i)**2)+(y(i)**2)-phi_(i) !+(0.1-1)*(0.61**2))-phi_(i)
!		f_(i) =0
 !       f_(i) = rho_(i) +(x(i)**2)+(y(i)**2 +(0.5-1)*(0.61**2))-phi_(i)

!         f_(i) = -4+(x(i)**2)+(y(i)**2)-phi_(i)
!		f_(i) = Zero

 !  Phi(ni,nj) = ((p_basic(1, nj+(ni-1)*nny))**2+(p_basic(2, nj+(ni-1)*nny))**2) +(0.1-1)*(0.61**2)
  !        else 
  !              Phi(ni,nj) = ((p_basic(1, nj+(ni-1)*nny))**2+(p_basic(2, nj+(ni-1)*nny))**2)*0.1
